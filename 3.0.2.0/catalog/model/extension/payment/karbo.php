...
class ModelExtensionPaymentKarbo extends Model {

  public function getMethod($address, $total){
    $this->language->load('extension/payment/karbo');

    $query = NULL;
    $sql_q = "";
    $sql_q  = "SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE";
    $sql_q .= " `geo_zone_id` = " . (int)$this->config->get('payment_karbo_geo_zone_id') . " AND";
    $sql_q .= " `country_id` = " . (int)$address['country_id'] . " AND";
    $sql_q .= " (`zone_id` = " . (int)$address['zone_id'] . " OR zone_id = 0)";
    $query = $this->db->query($sql_q)->rows;

    if ($total > 0 and ($this->config->get('payment_karbo_geo_zone_id') == 0 or count($query) == 1)){
      $status = true;
      } else {
      $status = false;
    }
            ...
            if (is_array($payinfo)){
              if ($payinfo['status']){
                ...
                if ($payinfo['amount'] >= $order_info['total']){
                  ...
                  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_karbo_order_payment_status_id'), $comm, true);
